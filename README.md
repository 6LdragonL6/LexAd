...
- 保证重新提交、引擎执行、法务详情和历史回溯使用同一版本快照；
- 支持品牌多行业、物料行业建议与管理员确认；
...
- 后端完整回归：119 项测试通过；
- 前端：TypeScript 检查通过；生产构建需在允许 esbuild 子进程的环境执行；
- 核心覆盖：候选不直接定性、引用验证、核验分流、平台降级、双轴法务复核、品牌行业候选、权限与版本追溯。
...
| 版本评估 | [v0.7.1 发布说明](docs/releases/v0.7.1-release-notes.md) · [历史归档](docs/archive/) |
...
| 版本 | 日期 | 主要变化 |
| --- | --- | --- |
| v0.7.1 | 2026-07-15 | 重提快照一致性、品牌多行业候选、法务双轴报告与主中栏审核体验 |
