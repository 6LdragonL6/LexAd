...
Each service function is independently callable; this module chains them together
for the full review pipeline.
"""
...
def run_full_pipeline(
    ...
    """Run the complete review pipeline from raw input to final response.

    Args:
        raw_text: Raw advertising text.
        image_file: Optional uploaded image.

    Returns:
        StandardResponse with all sections populated.
    """
    # 1. Preprocess
...
def _collect_matched_rule_ids(review) -> list[str]:
    """Collect all matched rule IDs across the three review layers."""
    ids: list[str] = []
