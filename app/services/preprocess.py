"""Preprocess service: handles text + optional image, returns preprocess result.

Independent, callable without request/response objects.
"""
...
def run_preprocess(raw_text: str, image_file: UploadFile | None = None) -> PreprocessResult:
    """Run preprocessing on raw text and optional image.

    Args:
        raw_text: Raw advertising text input (may be empty).
        image_file: Optional uploaded image file.

    Returns:
        PreprocessResult with OCR text, image summary, status, and warnings.
    """
    warnings: list[str] = []
    ocr_text = ""
    image_summary = ""
    ...
    if image_file is not None and image_file.filename:
        ...
        # Save to temp file for OCR processing (cross-platform)
        try:
