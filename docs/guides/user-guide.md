...
提交后系统创建后台任务，页面自动刷新并展示“物料已接收、法规审核、舆情审核、汇总审查结果”四个真实阶段。阶段只依据后端状态变化，不按等待时间伪造完成进度。
...
舆情结果独立展示风险等级、舆情安全分、议题、受影响群体、传播诱因、原文证据和建议。舆情安全分为 0–100，越高越安全，与法规合规分方向一致；无法形成可靠自动结论时显示“待人工复核”，不显示伪造的 0 分。
...
当 AI 已确认至少一项风险而法务选择“通过”时，系统会显示二次确认，列出风险数量和最高等级。只有再次点击“确认带风险通过”才会提交；退回、有条件通过和无风险通过不触发该弹窗。
...
从工作台打开的结果会返回工作台，从法务审核台打开的详情会返回法务审核台；浏览器原生后退保持可用。法务决定提交成功后不会通过后退重新打开可重复提交的旧表单。
...
品牌档案顶部的“现有记忆印象”根据历史审核固定归纳整体表现、近期法务倾向、平均修改轮次、高频风险和常见修改建议。少于 3 次法务决定时只显示“样本积累中”，不会形成稳定评价。该记忆与审核界面的品牌参考使用同一数据，但始终只是历史参考，不参与当前物料的自动审查评分。
