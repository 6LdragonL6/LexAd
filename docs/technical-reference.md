...
本文承接首页 README 中不适合展开的技术细节。当前版本为 `v0.6.3`。
...
| 舆情风险 | 使用 34 条真实案例规范形成的资料库；触发词只召回候选，本地证据与经过原文校验的 AI 判断按可信度融合 |

法律合规审查与舆情审查使用同一模型服务和统一 API 配置，内部按任务类型区分提示词与解析逻辑。业务请求必须声明 JSON 契约和示例；管理员配置测试只验证连接，不替代业务请求测试。内置舆情数据源为 `data/sentiment_cases.json`，启动同步、管理员导入和命令行导入共用统一规范化服务。
