...
本文记录 v0.7.2 当前实现、接口与运行约束。审查判断原则见 [审查方法论](review-methodology.md)，本地安装与排错见 [本地开发指南](../guides/local-development.md)。
...
法律与平台风险只接受存在于原文中的语义完整证据和有效资料标识。`legal_compliance_score` 与 `public_opinion_safety_score` 均为 0–100 且越高越好；后者在无可靠自动结论时为空。四阶段 `stages` 由真实任务与模块状态组装，不使用时间模拟。计分、状态和边界详见 [审查方法论](review-methodology.md)。
...
- `reviews`：法规合规分、可空舆情安全分、AI 结果、分模块状态、资料版本标识和法务决定；
...
| `COMPETITION_MODE` | `false` | 开启后资料中心、回收站和服务器 AI 配置只读，Render 环境变量成为唯一 AI Key 来源 |
| `LOGIN_RATE_LIMIT_ATTEMPTS` | `5` | 同一来源 IP 或用户名在限流窗口内允许的失败次数 |
| `LOGIN_RATE_LIMIT_WINDOW_SECONDS` | `600` | 登录失败限流窗口，单位秒 |
| `AI_RATE_LIMIT_REQUESTS` | `10` | 同一来源 IP 或账号在限流窗口内允许触发的 AI 任务数 |
| `AI_RATE_LIMIT_WINDOW_SECONDS` | `600` | AI 任务限流窗口，单位秒 |
| `DEMO_SEED_ENABLED` | `false` | 生产环境是否创建并恢复三个竞赛演示账号 |
| `DEMO_*_PASSWORD` | 空 | Render 注入的三个演示账号密码，至少 12 位；管理员密码必须独立 |
...
c9e7a1b4d2f0  v0.7.2 法规合规分与舆情安全分
```
...
v0.7.2 发布验证应同时覆盖：评分迁移循环、双正向评分、真实四阶段进度、带风险通过确认、来源一致返回、浏览器后退、明暗主题，以及既有候选裁决、引用校验、人工复核、角色权限和历史版本追溯。

## 13. 部署注意事项

- `APP_ENV=production` 时必须显式设置 `DATABASE_MODE=neon` 和有效的 `DATABASE_URL`。
- 生产环境必须设置稳定且随机的 `SECRET_KEY`。
- 公开竞赛部署应设置 `COMPETITION_MODE=true`，DeepSeek Key 只通过后端环境变量注入；完整流程见 [Render + Neon 竞赛部署指南](../guides/render-neon-deployment.md)。
